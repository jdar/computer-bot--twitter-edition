...
class Helper
  ...
  def self.didwhen(old_time) # stolen from http://snippets.dzone.com/posts/show/5715
    ...
    result = case val
      when 0..10;               'just a moment ago'
      when 0..40;               'less than ' + (val * 1.5).to_i.to_s.slice(0,1) + '0 seconds ago'
      when 0..60;               'less than a minute ago'
      when 0..(60 * 1.3);       "1 minute ago"
      when 0..(60 * 50);        "#{(val / 60).to_i} minutes ago"
      when 0..(60  * 60  * 1.4); 'about 1 hour ago'
      when 0..(60  * 60 * (24 / 1.02)); "about #{(val / 60 / 60 * 1.02).to_i} hours ago"
      else
        old_time.strftime("%H:%M %p %B %d, %Y")
     end
    return "#{result}"
  end # self.didwhen
